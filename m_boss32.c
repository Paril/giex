...
void makronBFG (edict_t *self)
{
	...
	if (self->radius_dmg)
		damage = 250 + 24 * self->monsterinfo.skill;
	else
		damage = 175 + 12 * self->monsterinfo.skill;
...
//	VectorCopy (self->enemy->s.origin, vec);
	...
	monster_fire_bfg (self, start, dir, damage, 400 + 20 * self->monsterinfo.skill, 100, 140 + 3 * self->monsterinfo.skill, MZ2_MAKRON_BFG);
}	
...
void MakronRailgun (edict_t *self)
{
	...
	if (self->radius_dmg)
		damage = 230 + 30 * self->monsterinfo.skill;
	else
		damage = 115 + 15 * self->monsterinfo.skill;
...
//	VectorCopy (self->enemy->s.origin, vec);
	...
	if (self->s.frame <= FRAME_attak413)
		dir[1] = self->s.angles[1] - (2 - 0.06 * self->monsterinfo.skill) * (self->s.frame - FRAME_attak413);
	else
		dir[1] = self->s.angles[1] + (2 - 0.06 * self->monsterinfo.skill) * (self->s.frame - FRAME_attak421);
