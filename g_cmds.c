...
int getItemInfoByName (char *name);
int getPowerupInfoByName (char *name);

void powerup_make_touchable (edict_t *item);
void powerup_make_touchable2 (edict_t *item);
...
//		Cmd_wornItems(ent);
	...
	else if (Q_strcasecmp(cmd, "cheat_giveitem") == 0)
	{
		if (!sv_cheats->value)
			gi.cprintf (ent, PRINT_HIGH, "You must run the server with '+set cheats 1' to enable this command.\n");
		else if (gi.argv(1));
		{
			edict_t *item, *targ = ent;
			iteminfo_t *info = getItemInfo(getItemInfoByName (gi.argv(1)));
			int item_id = getItemInfoByName (info->name);
	...
	else if (Q_strcasecmp(cmd, "cheat_givepowerup") == 0)
	{
		edict_t *powerup, *targ = ent;
		powerupinfo_t *info = getPowerupInfo(getPowerupInfoByName (gi.argv(1)));
		int item_id = getItemInfoByName (info->name);
		...
		if (info)
		{
			...
			powerup->nextthink = level.time + 1;
			powerup->solid = SOLID_TRIGGER;
			powerup->radius_dmg = getPowerupInfoByName (info->name);
			powerup->dmg_radius = info->maxlevel;
			powerup->owner = NULL;
