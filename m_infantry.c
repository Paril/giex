...
void InfantryMachineGun (edict_t *self)
{
	...
	if (self->radius_dmg)
		monster_fire_bullet (self, start, forward, (int) ceil(5.6 + 2.4 * self->monsterinfo.skill), 4, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_number);
	else
		monster_fire_bullet (self, start, forward, (int) ceil(2.8 + 1.2 * self->monsterinfo.skill), 4, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_number);
}
